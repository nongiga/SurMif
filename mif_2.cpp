...
Protein::Protein(string filename){
  min_x=1000.00, min_y=1000.00, min_z=1000.00;
  max_x=-1000.00, max_y=-1000.00, max_z=-1000.00;
...
void Protein::readPDB(string filename){
  ifstream ifs;
  ofstream ofs;
  string line, fields[13], thisresnumc, thisresnumcShort;
  ...
  while(ifs.good()){
    ...
    if(line.compare(0,6,"ATOM  ") == 0 || line.compare(0,6,"HETATM") == 0){
      ...
      //Remove spaces
      for (i=1;i<7;i++){  stripSpace(fields[i]);  }

      if(fields[4].compare("HOH")==0){ continue; } //Continue if its a water molecule line

      atom atm;
      atm.x=atof(fields[8].c_str());
      atm.y=atof(fields[9].c_str());
      atm.z=atof(fields[10].c_str());
      atm.chain=fields[5];
      atm.alt=fields[3];
      atm.atomnb=atoi(fields[1].c_str());
      atm.resnb=atoi(fields[6].c_str());
      atm.atomn=fields[2];
      atm.resn=fields[4];
      atm.bs=0;
      atm.mif=1;
      ...
      if(atm.alt.empty()){ atm.alt="-"; } //If there is no alterante location for this atom set alt to '-'
      if(atm.chain.empty()){ atm.chain="-"; } //If there is no chain for this atom set chain to '-'
      ...
      minx=roundCoord(atm.x-0.5,0);
      miny=roundCoord(atm.y-0.5,0);
      minz=roundCoord(atm.z-0.5,0);
      maxx=roundCoord(atm.x+0.5,1);
      maxy=roundCoord(atm.y+0.5,1);
      maxz=roundCoord(atm.z+0.5,1);
  ...
  cout<<"Number of ligand atoms: "<<LIGAND.size()<<endl;
}
