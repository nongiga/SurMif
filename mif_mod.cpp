//**this is the SurMif extension that Noga is working on
...
int main(int argc, char **argv)
{
  ...
  if(ProcessCmdLine(argc, argv)==24) return(0);
  //On vérifie s'il faut calculer les Mifs pour plusieurs protéines (-pp mon_fichier_texte)
  if(pairwiseF.compare("")!=0) getPairwise();//Si un fichier contenant des paires protéine - cavité (getcleft) est défini,on store les paires dans le vecteur px
  else pw.push_back(pwRun(proteinFile, cleftFile, resnumc, ligFile));//S'il n'y a pas de fichier défini avec -pp
...
int ProcessCmdLine(int argc, char **argv){
  ...
  if (readCmdLine(argc, argv))  return(24);
...
;
    ...
    if(line.compare(0,6,"ATOM  ") == 0 || line.compare(0,6,"HETATM") == 0){
      ...
      atom atm=atom(x, y, z, fields[4], fields[2], fields[5], fields[3], resnb, atomnb, 0, 0, 1);
      ...
      //find when not found return atomTypes.end() (that is the comparison)
      ...
      //cout << atm.mif << " apres" << endl;
      //for each resnumc, create resnumcShort to pass in LIGAND
      
      for(int i=0; i<nbResnumc; i++){
        resnumc = resnumcList[i];
        resnumcShort = resnumcList[i];
        resnumcShort = resnumcShort.substr(0,resnumcShort.length()-1);
  ...
  cout<<"Number of atoms: "<<LIGAND.size()<<endl;
}

void Protein::getAtomDir(){
  int i,found,ring,needRef,rDir;
  float xr,yr,zr;
  string tatomn, tatomn2;
    ...
    if(PROTEIN[i].mif==1){ //If its an atom we want to consider for the Mif calculations
      ...
      if(needRef==1){
        if(getRefAtom(xr, yr, zr, PROTEIN[i].resn, PROTEIN[i].resnb, tatomn, tatomn2, ring, PROTEIN[i].x, PROTEIN[i].y, PROTEIN[i].z, PROTEIN[i].chain)==1){
          PROTEIN[i].setRadial(xr, yr, zr, 1);
          PROTEIN[i].rDir=rDir;
        }
      } else PROTEIN[i].setRadial(0,0,0,0);
    }
  }
...
Grid::Grid(string filename, Protein& prot){
  vrtx050=0, vrtx100=0, vrtx150=0, vrtx200=0;

  //Build the grid everywhere around the protein and classify them as protein vertices (p=1) or probe vertices (p=0)
  if(filename.empty()) buildGrid(prot);
  else readGetCleft(filename, prot.PROTEIN, prot.LIGAND); //Create grid using the getcleft spheres
...
int Grid::generateID(int w, int h, int x, int y, int z){  return(z * (w * h) + y * w + x);  }
...
int Grid::buildGrid(Protein& prot){ //Building the grid using the whole protein
  ...
  int id;
  for(float x=gmin.x; x<=gmax.x; x+=stepsize){
    for(float y=gmin.y; y<=gmax.y; y+=stepsize){
      for(float z=gmin.z; z<=gmax.z; z+=stepsize){
       ...
       // cout << "avant boucle" << endl;
        ...
        id=generateID(width,height,(int)((x-gmin.x)/gridStep)+1,(int)((y-gmin.y)/gridStep)+1,(int)((z-gmin.z)/gridStep)+1);
        it = GRID.find(id);

        if(it == GRID.end()){ //If we never added this point to the vector
          vertex vrtx=vertex(x,y,z);
          ...
          if(pg==1){ //If its a grid vertex that represents a protein vertex
            ...
            vrtx.grid[0]=(inGridRes(vrtx,2.0)==1)?1:0;
            vrtx.grid[1]=(inGridRes(vrtx,1.5)==1)?1:0;
            vrtx.grid[2]=(inGridRes(vrtx,1.0)==1)?1:0;
            vrtx.grid[3]=(inGridRes(vrtx,0.5)==1)?1:0;
...
int Grid::readGetCleft(string filename, vector<atom>& protVec, vector<float>& ligVec){
  ...
  while(ifs.good()){
    ...
    if(line.compare(0,6,"ATOM  ") == 0 || line.compare(0,6,"HETATM") == 0){
      ...
      minx=roundCoordDown(x-rad);
      miny=roundCoordDown(y-rad);
      minz=roundCoordDown(z-rad);
      maxx=roundCoordUp(x+rad);
      maxy=roundCoordUp(y+rad);
      maxz=roundCoordUp(z+rad);
      ...
      for(nx=minx; nx<=maxx; nx+=gridStep){
        for(ny=miny; ny<=maxy; ny+=gridStep){
          for(nz=minz; nz<=maxz; nz+=gridStep){
            ...
            //Generate grid vertex ID
            id=generateID(width,height,(int)((nx-gmin.x)/gridStep)+1,(int)((ny-gmin.y)/gridStep)+1,(int)((nz-gmin.z)/gridStep)+1);
            it = GRID.find(id);
            ...
            if(resnumc.compare("")!=0){ //If we must remove grid points too far from the anchor atoms
              ...
              if(minDist>gridLigDist){
               // cout<<nx<<" "<<ny<<" "<<nz<<" to far "<<minDist<< " plus grand que " << gridLigDist<<endl;
                continue;
              }
            ...
            vertex vrtx=vertex(nx, ny, nz, 0, 0, id);
            ...
            //Calculate in which grid resolution this vertex is
            if(inGridRes(vrtx,2.0)==1) vrtx.grid[0]=1;
            else vrtx.grid[0]=0;

            if(inGridRes(vrtx,1.5)==1) vrtx.grid[1]=1;
            else vrtx.grid[1]=0;

            if(inGridRes(vrtx,1.0)==1) vrtx.grid[2]=1;
            else vrtx.grid[2]=0;

            if(inGridRes(vrtx,0.5)==1) vrtx.grid[3]=1;
            else vrtx.grid[3]=0;
  ...
  int tooClose=0, tooFar=0, badRes=0, chipped=0;
  ...
  while(it!=GRID.end()){
    ...
    else if(it->second.grid[zip]==0 && zip!=-1){
      GRID.erase(it++);
      chipped++;
      badRes++;
    }
 ...
 void Grid::getBuriedness(){
   ...
   while(m!=GRID.end()){
     ...
     int xi=(int)((m->second.x-gmin.x)/gridStep)+1, yi=(int)((m->second.y-gmin.y)/gridStep)+1, zi=(int)((m->second.z-gmin.z)/gridStep)+1;

     // cout<<xi<<" "<<yi<<" "<<zi<<endl;
     // cout<<gmin.x<<" | "<<xl<<" "<<m.x<<" "<<xr<<" | "<<gmax.x<<" || "<<width<<endl;
     // cout<<gmin.y<<" | "<<yd<<" "<<m.y<<" "<<yu<<" | "<<gmax.y<<" || "<<height<<endl;
     // cout<<gmin.z<<" | "<<zb<<" "<<m.z<<" "<<zf<<" | "<<gmax.z<<" || "<<depth<<endl;
     ...
     for(int tx=xi-1; tx>=1; tx--){
       ...
       if(GRID.find(id) != GRID.end() && GRID[id].p==1){
         bu++;
         break;
       }
     }
...
float roundCoordDown(float number){ 
  //min_or_max (1=max, 0=min)
  int rounded=(int)(number/gridStep);
  float new_coord=rounded*gridStep;
  if(new_coord>number || fabs(new_coord-number)<0.0001) return (rounded-1)*gridStep;
  else return(new_coord);
}

float roundCoordUp(float number){ 
  //min_or_max (1=max, 0=min)
  int rounded=(int)(number/gridStep);
  float new_coord=rounded*gridStep;
  if (new_coord<number || fabs(new_coord-number)<0.0001) return (rounded+1)*gridStep;
...
int Grid::inGridRes(vertex& vrtx, float res){
  ...
  float x_diff=fabs(vrtx.x-gmin.x)/res, x_ref=round(x_diff);
  float y_diff=fabs(vrtx.y-gmin.y)/res, y_ref=round(y_diff);
  float z_diff=fabs(vrtx.z-gmin.z)/res, z_ref=round(z_diff);
...
void Grid::smooth(){
  ...
  for(git=this->GRID.begin(); git!=this->GRID.end(); git++){
    ...
    int id;
    vertex& m=git->second;
    if(m.p==1 || m.grid[0]!=1) continue;
    int xi=(int)((m.x-gmin.x)/gridStep)+1, yi=(int)((m.y-gmin.y)/gridStep)+1, zi=(int)((m.z-gmin.z)/gridStep)+1;


    for(int tx=xi-smoothDist; tx<=xi+smoothDist; tx++){
      id=generateID(width,height,tx,yi,zi);
      if(this->GRID.find(id) != this->GRID.end()){
    ...
    int tx=xi-smoothDist, ty=yi+smoothDist, tz=zi+smoothDist;
    for(int s=0; s<6; s++){
      tx++; ty--; tz--;
...
int is_coord_in_cube(float x, float y, float z, float center_x, float center_y, float center_z, float edge){
  float x_max=center_x+edge, x_min=center_x-edge, y_max=center_y+edge, y_min=center_y-edge, z_max=center_z+edge, z_min=center_z-edge;
    ...
    if(it->second.p==1){
      ...
      if((zip!=-1 && it->second.grid[zip]==1) || zip==-1){
        ...
        for(probe=0; probe<nbOfProbes; probe++){
          ...
          if(it->second.nrgs[probe]==0 || abs(it->second.nrgs[probe]-0.0)<0.00001)  fprintf(fpNew, " 0");
          else fprintf(fpNew, " %6.3f",it->second.nrgs[probe]);
          
          if(it->second.angles[probe]==0 || abs(it->second.angles[probe]-0.0)<0.00001) fprintf(fpNew, " 0");
          else fprintf(fpNew, " %6.3f",it->second.angles[probe]);
        }
        for(i=0; i<4; i++) fprintf(fpNew, " %1d",it->second.grid[i]);
        // for(i=0; i<aa.size(); i++){
...
void getPseudoC(){
  ...
  ifstream infile(fn.c_str());
  string line, res, atom, pseudo;

  //cout << "Getting PseudoC Reference.." << endl;
...
void getEpsilons(){
  string fn;
  if(matrixF.compare("")!=0) fn=matrixF;
  else fn=basePath + "/forcefield_files/"+ff+"/epsilons";
  
  ifstream infile(fn.c_str());
  string s;
  int row=0;
  vector<string> coln;
  //cout <<"Getting Epsilon Reference.." << endl;
  ...
  while(getline(infile,s)){
    ...
    copy(istream_iterator<string>(iss), istream_iterator<string>(), back_inserter<vector<string> >(tokens));
    if(row==0){
      for(int i=0; i<tokens.size(); i++) coln.push_back(tokens[i]);
    } else{
...
void readLigFile(){
  ...
  string s;
  cout << "Getting ligand File Reference.." << endl;
  while(getline(infile,s)){
    istringstream iss(s);
    vector<string> tokens;
    copy(istream_iterator<string>(iss), istream_iterator<string>(), back_inserter<vector<string> >(tokens));
...
void getProbes(){
  string fn;
  if(probesF.compare("")!=0) fn=probesF;
  else fn=basePath + "/forcefield_files/"+ff+"/probes";

  ifstream infile(fn.c_str());
  string line, pbn;
  float pmin, pmax, thresh;

  //cout <<"Getting Probes Reference.." << endl;
  while(getline(infile,line)){
    stringstream test(line);
    test >> pbn >> pmin >> pmax >> thresh;
    minD[pbn]=pmin;
    maxD[pbn]=pmax;
    nrgT[pbn]=thresh;
...
void getEnv(map<int,vertex>& grid, vector<atom>& prot, vector<int>& vrtxList){
  ...
  for(int i=0; i<vrtxList.size(); i++){
    ...
    if(m.p==1  || (m.grid[0]!=1 && m.grid[1]!=1 && m.grid[2]!=1 && m.grid[3]==1)) continue;
  ...
  for(int i=0; i<vrtxList.size(); i++){
    ...
    if(m.grid[0]==1 || m.grid[1]==1 || m.grid[2]==1 || m.grid[3]==1){ //If its in one of the grid resolution (not sure if this is useful anymore)
      ...
      for(int probe=0; probe<nbOfProbes; probe++){ //Iterate each probe
        ...
        float enrg_sum=0.00, closest=10000;
        int countAtms=0;
        ...
        if(countAtms>0){
          ...
          if(enrg_sum<nrgT[probes[probe]] || (fabs(enrg_sum-nrgT[probes[probe]]))<0.001){
            m.ints[probe]=1;
            for(int gi=0; gi<4; gi++){ if(m.grid[gi]==1)  ssm[gi]++; }
            flag=1;            
          }
          if(printDetails==1) cout<<m.grid[0]<<" "<<m.grid[1]<<" "<<m.grid[2]<<" "<<m.grid[3]<<" probe "<<probe<<" nrg "<< enrg_sum<<" int "<<m.ints[probe]<<endl;
        }else if(printDetails==1) cout<<"NRG "<<probe<<" no atoms"<<endl;
      }

      //Increment search space
      if(flag==1){
        for(int gi=0; gi<4; gi++){  if(m.grid[gi]==1)  ss[gi]++;  }
      }
    }
  }
}
...
double calcNrg(vertex& vrtx, atom& atm, int pbId, int& count_atoms, float& closest){
  float dist,alpha=1.0,epsilon,angle=1.0,rDist,rpDist;
  double energy=0.0;
  ...
  string at=atomTypes[atm.resn+"_"+atm.atomn], pat=probes[pbId];
  ...
  if(((acc[at]==1 && don[pat]==1) || (acc[pat]==1 && don[at]==1)) && atm.dir==1){
    ...
    }else if(printDetails==1) cout<<"Both aromatic but no angle to calculate."<<endl;
  }else if(printDetails==1){
    if(chr[at]==1 && chr[pat]==1) cout<<"Charged couple "<<endl; //if the interaction is charged
    else if(hyd[pat]==1 && hyd[at]==1) cout << "Hydrophobic "<<endl; //if its a hydrophoic probe 
  }
  count_atoms++;
...
void getPseudo(map<int,vertex>& grid, vector<atom>& prot, vector<int>& vrtxList){
  ...
  for(int j=0; j<prot.size(); j++){
    ...
    if(it != pseudoC.end()){
      ...
      int angleType=0;
      if(pseudo.compare("doa")==0 || pseudo.compare("don")==0 || pseudo.compare("acc")==0) angleType=1;
      ...
      for(int i=0; i<vrtxList.size(); i++){
        ...
        if(m.bu<bul || m.grid[2]!=1) continue;
        ...
        // else if(angleType==2){ //aromatic angle
          ...
          // cout<< " angle "<<angle<<" dist "<<dist<<endl;
        // }
        if(dist<bestD){ bestD=dist; bestID=i; }
      }
      if(bestID!=-1)
        pseudoList.push_back(pseudovrtx(grid.at(vrtxList.at(bestID)).x, grid.at(vrtxList.at(bestID)).y,grid.at(vrtxList.at(bestID)).z, bestD, pseudo));
    }
  }
}

void getStats(map<int,vertex>& grid, vector<atom>& lig,vector<int>& vrtxList){
  ...
  for(int probe=0; probe<nbOfProbes; probe++){ //Iterate each probe
      ...
      if(lig[l].at.compare(idAt[probe])==0){
        for(int i=0; i<vrtxList.size(); i++){
          ...
          if((zip!=-1 && it->second.grid[zip]==1) || zip==-1){
            ...
            pbV.push_back(pbVrtx(dist, m.nrgs[probe]));
    ...
    if(pbV.size()!=0){
      ...
      int ctp=0, ctn=0, rocV[tn];
      rocV[0]=0;
      for(int i=0; i<pbV.size(); i++){
        cout<<idAt[probe]<<" "<<pbV.at(i).dist<<" "<<pbV.at(i).nrg<<endl;
        if(pbV.at(i).dist<distpbV) { cout<< " TP"; ctp++; }
        else ctn++;
        rocV[ctn]=ctp;
      }

      float xs=1.0/ctn, ys=1.0/ctp, roc=0.0;
      // cout<<"ctp "<<ctp<<" ctn "<<ctn<<" xs "<<xs<<" ys "<<ys<<endl;
      for(int j=1; j<=tn; j++) roc+=(xs*((float)rocV[j-1])*ys)+(xs*(((float)rocV[j]*ys)-((float)rocV[j-1]*ys)))/2;
      // cout<<j<<" "<<rocV[j]<<endl;
...
bool compByNrg(pbVrtx a, pbVrtx b) {  return a.nrg < b.nrg; }

void getPairwise(){

  cout<<"Getting pairwise"<<endl;
  string line;
...
void printMenu(){
  cout << "\n!---   IsoMIF   ---!\nWelcome.Bienvenue" <<endl;
  cout << "-p <filename>        : \t Protein (PDB format)" <<endl;
  cout << "-g <filename>        : \t Cleft of protein (GetCleft sphere format)" <<endl;
  cout << "-pp <filename>       : \t File with highthroughput mif calculations" <<endl;
  cout << "-s                   : \t Which grid resolution to consider [0.25, 0.5, 1.0 or 2.0] default is 0.5 Angstrom" <<endl;
  cout << "-d <distance in Ang.>: \t atom-probe distance threshold. (default: "<< atmPbMaxDist <<" Angstrom)" <<endl;
  cout << "-c <chain>           : \t PDB chains to consider for grid and energy calculations" <<endl;
  cout << "-w <distance in Ang.>: \t minimum distance of the probes to the atoms (default: "<< minGridDist <<" Angstrom)" <<endl;
  cout << "-v <distance in Ang.>: \t maximum distance of the probes to the atoms (default: "<< maxGridDist <<" Angstrom)" <<endl;
  cout << "-pr                  : \t print details of energy calculations for each probe-atom pair" <<endl;
  cout << "-o                   : \t output directory" <<endl;
  cout << "-b                   : \t burriedness level (0-14) [14]" <<endl;
  cout << "-m                   : \t smooth" <<endl;
  cout << "-m                   : \t vcontacts file" <<endl;
  cout << "-ff                  : \t force field to use" <<endl;
  cout << "-mat                 : \t epsilon matrix to use" <<endl;
  cout << "-pb                  : \t probes threshold file to use" <<endl;
  cout << "-a                   : \t angle threshold" <<endl;
  cout << "-t                   : \t filename (tag) [pdb name by default]" <<endl;
  cout << "-lf                  : \t ligand file with atom types" <<endl;
  cout << "-dpv                 : \t distance probe-Vrtx to consider a vrtx to be true-positive" <<endl;
  cout << "-sf                  : \t stats output file" <<endl;
  cout << "-l                   : \t RESNUMC of the ligand from which to crop the grid" <<endl;
  cout << "-r                   : \t maximum distance between the grid and the ligand or residues atoms" <<endl;
  cout << "-x                   : \t do not write atoms in mif file" <<endl;
  cout << "-og                  : \t dir for grid file" <<endl;
...
int readCmdLine(int argc, char **argv){
  ...
  if(argc<nbArgumentNecessaire){
    if (argc!=1){
      cout << "Missing obligatory arguments. Menu below:" << endl;
    }
    printMenu();
    return(24);
  }
  ...
  for(nb_arg=1; nb_arg<argc; nb_arg++){
    ...
    if(strcmp(argv[nb_arg],"-h")==0){ printMenu(); return(24); }

    else if(strcmp(argv[nb_arg],"-p")==0){ proteinFile=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-g")==0){ cleftFile=string(argv[nb_arg+1]); }
   
    else if(strcmp(argv[nb_arg],"-x")==0){ printAtoms=0; }
    
    else if(strcmp(argv[nb_arg],"-o")==0){ outBase=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-ff")==0){  ff=string(argv[nb_arg+1]);  }
    
    else if(strcmp(argv[nb_arg],"-sf")==0){  statsF=string(argv[nb_arg+1]);  }

    else if(strcmp(argv[nb_arg],"-c")==0){ chain=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-lf")==0){ ligFile=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-mat")==0){ matrixF=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-pb")==0){ probesF=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-m")==0){ sscanf(argv[nb_arg+1], "%d", &smoothDist);  }
    
    else if(strcmp(argv[nb_arg],"-v")==0){ sscanf(argv[nb_arg+1], "%f", &maxGridDist); }

    else if(strcmp(argv[nb_arg],"-w")==0){ sscanf(argv[nb_arg+1], "%f", &minGridDist); }

    else if(strcmp(argv[nb_arg],"-a")==0){  sscanf(argv[nb_arg+1], "%f", &angThresh); }
    
    else if(strcmp(argv[nb_arg],"-r")==0){  sscanf(argv[nb_arg+1], "%f", &gridLigDist); }
    
    else if(strcmp(argv[nb_arg],"-d")==0){  sscanf(argv[nb_arg+1], "%f", &atmPbMaxDist); }
    
    else if(strcmp(argv[nb_arg],"-dpv")==0){  sscanf(argv[nb_arg+1], "%f", &distpbV); }
    
    else if(strcmp(argv[nb_arg],"-b")==0){  sscanf(argv[nb_arg+1], "%d", &bul); }

    else if(strcmp(argv[nb_arg],"-og")==0){ outGridBase=string(argv[nb_arg+1]); }
    
    else if(strcmp(argv[nb_arg],"-pr")==0){ printDetails=1; }
    
    else if(strcmp(argv[nb_arg],"-t")==0){  tag=string(argv[nb_arg+1]); }

    else if(strcmp(argv[nb_arg],"-pp")==0){ pairwiseF=argv[nb_arg+1]; }

    else if(strcmp(argv[nb_arg],"-l")==0){
      if(sscanf(argv[nb_arg+1],"%d", &nbResnumc) !=0){
        nb_arg++;
        for(int h=0; h<nbResnumc; h++){ resnumcList[h] = argv[++nb_arg];  }
      }
      else {  resnumcList[0] = argv[nb_arg+1];  }
    }

    else if(strcmp(argv[nb_arg],"-s")==0){
      sscanf(argv[nb_arg+1], "%f", &stepsize);
      if(stepsize == 0.5){  zip=3;  }
      else if(stepsize == 1.0){ zip=2;  }
      else if(stepsize == 1.5){  zip=1; }
      else if(stepsize == 2.0){ zip=0;  }
    }
...
void initializeInfo(int pwr){
    ...
    size_t end=proteinFile.find(".pdb");
    int i=end-1;
    while (tag.empty()) {
      if(proteinFile.at(i)=='/') tag=proteinFile.substr(i+1,end-i-1);
      else if (i<=0) tag=proteinFile.substr(i,end-i);
      i--;
    }
}

void initializeBounds(){
  gmin.x=1000.00, gmin.y=1000.00, gmin.z=1000.00;
  gmax.x=-1000.00, gmax.y=-1000.00, gmax.z=-1000.00;
}
